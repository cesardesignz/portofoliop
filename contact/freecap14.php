...
// many thanks to http://ocr-experts.com and http://sam.zoy.org/pwntcha/ for testing
// for jpgs, 'transparent' is white
$bg_type = 1;
// should we blur the background? (looks nicer, makes text easier to read)
$blur_bg = true;
...
switch($bg_type)
{
	...
	case 2:
		...
		break;
	case 3:
		// around 50-60 provides best protection
...
//////////////////////////////////////////////////////
/*
  Not needed with Mike's Contact Script.  We already test for brute force attacks.
...
function sendImage($pic)
{
	...
	/*
	 *  I don't know why the origional author hid this, but here it is.  Keep it if you wish.
	 *  	- Mike
    *    Here's what it translates to: header("X-Captcha: freeCap 1.4 - www.puremango.co.uk");
	 */
...
if($use_dict==1)
{
	...
	$wordlen = $rand_func(4,$max_word_length);
...
if($bg_type!=0)
{
    ...
    if ($gd_version) {
       ...
       $im3 = ImageCreate($width, $height);
    }
    if ($gd_version) {
       $temp_bg = ImageCreateTrueColor($width*1.5,$height*1.5);
    } else {
       $temp_bg = ImageCreate($width*1.5,$height*1.5);
    }
	...
	if($bg_type==1)
	{
		...
		{
			if ($gd_version) { ImageSetThickness($temp_bg,$rand_func(2,6)); }
...
// so that's cool.
$tag_str = "freeCap v1.4";
// for debug:
//$tag_str = "[".$word."]";

// ensure tag is right-aligned
$tag_width = strlen($tag_str)*7;
// write tag
ImageString($im, 3, $width-$tag_width, $height-13, $tag_str, $tag_col);
